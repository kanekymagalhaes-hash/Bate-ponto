...
async function handleApi(req, res, url) {
  ...
  if (req.method === 'GET' && url.pathname === '/api/records') {
    ...
    const records = data.records.filter((record) => record.date.startsWith(month)).sort((a, b) => b.date.localeCompare(a.date));
    return sendJson(res, 200, { records, summary: getSummary(data.records, month) });
  }

  if (req.method === 'POST' && url.pathname === '/api/clock-in') {
    if (data.records.some((record) => record.type === 'work' && !record.endAt)) {
      return sendJson(res, 409, { error: 'Já existe uma jornada em andamento.' });
    }
    const record = { id: randomUUID(), type: 'work', date: dateKey(now), startAt: now, endAt: null, pauses: [], workedMinutes: 0 };
  ...
  if (req.method === 'POST' && url.pathname === '/api/clock-out') {
    const record = data.records.find((item) => item.type === 'work' && !item.endAt);
  ...
  if (req.method === 'POST' && url.pathname === '/api/leaves') {
    ...
    if (data.records.some((record) => record.date === body.date)) return sendJson(res, 409, { error: 'Já existe um registro para esta data.' });
    const record = { id: randomUUID(), type: 'leave', date: body.date, reason: String(body.reason || 'Folga') };
    data.records.push(record);
    await saveData(data);
    return sendJson(res, 201, { record });
  }
